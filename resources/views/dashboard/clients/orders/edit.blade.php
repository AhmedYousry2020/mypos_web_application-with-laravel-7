...
@section('content')
    <div class="content-wrapper">
        ...
        <div class="col-md-6">
            <div class="box box-primary">
                ...
                <h4 class="panel-title">
                     <a data-toggle="collapse" href="#{{str_replace(' ','-',$category->name) }} " class="btn-to-find-stock">{{ $category->name }}</a>
                </h4>
                </div>
        ...
        @foreach($category->products as  $product)
               <tr>
                <td>{{ $product->name }}</td>
                <td class="sto_no" data-str=" @lang('site.not_avaliable_products') ">{{ $product->stock }}</td>
                ...
                <td>{{ number_format($product->sale_price,2) }}</td>
                 ...
                 class="btn {{in_array($product->id,$order->products->pluck('id')->toArray()) ?'btn-default disabled' : 'btn-success add-product-btn' }} btn-sm "><i class="fa fa-plus"></i></a></td>
...
<tr>
    <td>{{$product->name}}</td>
    <input type="hidden" name="prdouct_ids[]" value='{{$product->id}}'>
...
<button class="btn btn-primary btn-block " id="form-btn"><i class="fa fa-edit"></i> @lang('site.edit_order')</button>
    ...
    <h3 class="box-title" style="margin-bottom: 10px">@lang('site.previous_orders')
        <small>{{ $client->orders->count() }}</small>
    </h3>
